<template>
  <div class="aircraft-view">
    ...
    <div class="table">
      ...
      <div class="row aircraft" v-for="(_aircraft, index) in aircraft" :key="index" @click.stop="selectAircraft(index)" >
        <div style="color:#000;" >
        ...
        <div :class="{old: _aircraft.installedVersion !== _aircraft.newestVersion}" >
          {{_aircraft.installedVersion}}
        </div>
        <div style="display:flex; justify-content: center;">
          <div class="status" :class="{
            locked: _aircraft.installLocked == true,
            notInstalled: _aircraft.installedVersion == null && _aircraft.installLocked == false,
            installed: _aircraft.installedVersion !== null && _aircraft.installLocked == false,
          }">
            <i class="fas fa-check" style="color:#fff"  v-if="_aircraft.selected" ></i>
          </div>
        </div>
      </div>
    </div>
...
export default Vue.extend({
  ...
  methods: {
    installAirfraft() {
      invoke({cmd:"installAircraft", names: Array.from(this.selectedAircraftNames)})
    },
