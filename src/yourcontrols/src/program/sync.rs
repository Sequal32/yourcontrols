...
impl SyncController {
    pub fn tick(
        ...
        if let Some(pending_action) = sim.definitions.get_next_pending_action() {
            SyncHandler::handle_program_action(pending_action, &mut client, network, sim, control);
        }
    ...
    fn handle_program_action(
        ...
        match pending_action {
            ProgramAction::TakeControls => {
                if !control.has_control() && !network.observing {
                    if let Some(in_control) = network.clients.get_client_in_control() {
                        control.take_control(&sim.conn, &sim.definitions.lvarstransfer.transfer);
