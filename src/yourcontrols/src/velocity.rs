...
use crate::util::{InDataTypes, Vector3};
use crate::varreader::SimValue;

fn get_vector3_from_data_map(data: &SimValue, key_prefix: &str) -> Option<Vector3> {
    Some(Vector3 {
        // x: data.get(&format!("{} X", key_prefix))?.get_as_f64(),
        // y: data.get(&format!("{} Y", key_prefix))?.get_as_f64(),
        x: 0.0,
        y: 0.0,
        z: data.get(&format!("{} Z", key_prefix))?.get_as_f64(),
    })
}
