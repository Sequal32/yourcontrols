...
use std::{collections::HashMap, net::{Ipv4Addr, SocketAddr, SocketAddrV4}, thread::sleep, time::{Duration, Instant}};
use yourcontrols_net::{Message, Payloads, SenderReceiver, get_socket_config};
...
impl Hoster {
    pub fn new(port: u16, communicator_address: SocketAddr) -> Self {
        let socket = Socket::bind_with_config(SocketAddrV4::new(Ipv4Addr::new(0, 0, 0, 0), port), get_socket_config(SERVER_TIMEOUT)).expect("Failed to bind!");
    ...
    fn handle_heartbeats(&mut self) {
        for (_, state) in self.servers.get_server_states().iter_mut() {
            if state.heartbeat_instant.elapsed().as_secs_f32() < 0.5 {return}
    ...
    fn process_communicator(&mut self) {
        match self.communicator.poll() {
            ...
            Ok(HosterPayloads::ClientConnecting { session_id, addr }) => {
                ...
                self.communicator.send_message(HosterPayloads::ClientAcknowledged {
                    addr
                });
            }
            _ => {}
    ...
    fn cleanup(&mut self) {
        if self.cleanup_timer.elapsed().as_secs() < 10 {return}

        for removed in self.servers.remove_unused() {
            self.communicator.send_message(HosterPayloads::SessionClosed {
                session_id: removed,
            });
        }
